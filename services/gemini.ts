...
/**
 * Dispatches a custom event when an API error occurs, allowing the UI to reset 
 * the key selection state if the platform reports a missing/invalid entity.
 */
const handleApiError = (error: any) => {
  window.dispatchEvent(new CustomEvent('gemini-api-error', { detail: error }));
  throw error;
};
